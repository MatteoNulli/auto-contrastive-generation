...
def calculate_contrasted_logits(upper_layer_logits, lower_layer_logits, minimum_candidates=1, alpha=0.1,  config=None):
...
def calculate_contrasted_logits_softmax(upper_layer_logits, config=None):
    from autocontrastive_gen.utils import device

    assert upper_layer_logits is not None
    probs = torch.softmax(upper_layer_logits, dim=-1)
...
def calculate_contrasted_logits_JSD(upper_layer_logits, lower_layer_logits, minimum_candidates=1, alpha=0.1):
    from autocontrastive_gen.utils import device
